...
static int ignore_error_type(struct thread_data *td, enum error_type_bit etype,
    ...
    while ((fname = strsep(&str, ":")) != NULL) {
        ...
        if (i == td->o.ignore_error_nr[etype]) {
            ...
            error = realloc(error, td->o.ignore_error_nr[etype] * sizeof(int));
        }
        if (fname[0] == 'E') {
    ...
    while ((cpu = strsep(&str, ",")) != NULL) {
        ...
        while (icpu <= icpu2) {
            if (icpu >= FIO_MAX_CPUS) {
                log_err(
                    "fio: your OS only supports up to"
                    " %d CPUs\n",
                    (int)FIO_MAX_CPUS);
                ret = 1;
                break;
...
static int zone_split_ddir(struct thread_options *o, enum fio_ddir ddir,
    ...
    if (perc < 100) {
        log_err(
            "fio: access percentage don't add up to 100 for zoned "
...
static int rw_verify(const struct fio_option *o, void *data) {
    ...
    if (read_only && (td_write(td) || td_trim(td))) {
        log_err(
            "fio: job <%s> has write or trim bit set, but"
...
static int gtod_cpu_verify(const struct fio_option *o, void *data) {
    ...
    if (td->o.gtod_cpu) {
        log_err(
            "fio: platform must support CPU affinity for"
...
struct fio_option fio_options[FIO_MAX_OPTS] = {
    ...
    },
    {
        ...
        .posval = {
            ...
            {.ival = "randrw", .oval = TD_DDIR_RANDRW, .help = "Random read and write mix"},
            {.ival = "trimwrite", .oval = TD_DDIR_TRIMWRITE, .help = "Trim and write mix, trims preceding writes"},
        },
    },
    {
        .name = "rw_sequencer",
...

    {
        ...
        .posval = {
            ...
            {.ival = "windowsaio", .help = "Windows native asynchronous IO"},
#endif
#ifdef CONFIG_RBD
            {.ival = "rbd", .help = "Rados Block Device asynchronous IO"},
#endif
            {.ival = "mmap", .help = "Memory mapped IO"},
...
#endif
#ifdef CONFIG_GFAPI
            {.ival = "gfapi", .help = "Glusterfs libgfapi(sync) based engine"},
            {.ival = "gfapi_async", .help = "Glusterfs libgfapi(async) based engine"},
#endif
#ifdef CONFIG_LIBHDFS
            {.ival = "libhdfs", .help = "Hadoop Distributed Filesystem (HDFS) engine"},
#endif
...
#endif
            {.ival = "nbd", .help = "Network Block Device (NBD) IO engine"},
#ifdef CONFIG_DFS
            {
                .ival = "dfs",
...
#endif
            {.ival = "truncate", .oval = FIO_FALLOCATE_TRUNCATE, .help = "Truncate file to final size instead of allocating"},
...
#error "FIO_HAVE_IOPRIO_CLASS requires FIO_HAVE_IOPRIO"
    ...
    },
    {
        ...
        .posval = {
            ...
            {.ival = "both", .oval = UNIFIED_BOTH, .help = "Statistics are reported normally, followed by the mixed statistics"},
            /* Compatibility with former boolean values */
            {
                .ival = "0",
                .oval = UNIFIED_SPLIT,
